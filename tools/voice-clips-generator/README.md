...
are idempotent. Cleaning doesn't touch `manifest.json` — the manifest hash
covers the transcript/model/voice, not the audio bytes, so a cleaned clip
won't be regenerated. Regenerating a clip does overwrite the cleaned file,
so run cleaning again after generating.

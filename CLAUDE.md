...
- **Multiplayer authoritative state:** lightweight Firebase Cloud Functions (`joinGame` and `submitAnswer`, in `firebase/functions/`) are the single source of truth for multiplayer game state, specifically to prevent client-side sync/visual inconsistencies that could confuse the student.
- **Cloud Function warmup:** `submitAnswer` sits in the latency-sensitive answer path but scales to zero when idle, so a cold start can delay a student's first answer by seconds. Whenever a player joins a previously empty room, `joinGame` returns `firstPlayer: true` and the client fires a fire-and-forget `{warmup: true}` call to `submitAnswer`, which returns early without touching game state — the cold start is absorbed at join time instead of on the first answer.
